        ...
        dados.forEach(p => {
            ...
            if (!markers[key]) {
                ...
                const m = L.marker([p.lat, p.lng], { 
                    icon: ic,
                    // NOVO: persistimos o status no marker para a clusterização
                    statusNagios: (p.status_nagios || '').toUpperCase()
                });
                ...
                // Atualizar status no marker (usado pelo cluster)
                markers[key].options.statusNagios = (p.status_nagios || '').toUpperCase();

                // Força o recálculo do ícone do cluster que contém este marker
                if (clusterGroup && markers[key]) {
                clusterGroup.refreshClusters(markers[key]);
                }
